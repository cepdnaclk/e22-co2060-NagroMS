...
const typeIcon = (type) => {
    const p = { size: 15, color: 'var(--exp-green)', strokeWidth: 1.8 };
...
const statConfig = [
    ...
    { key: 'activeFarmers', label: 'Connected Members', icon: <Users size={18} />, color: '#7c3aed' },
    { key: 'pendingConnections', label: 'Pending Requests', icon: <UserPlus size={18} />, color: '#dc2626' },
];
...
export default function ExpertDashboard() {
    ...
    const { stats, consultations, questions, connections, profile } = data || {};
    const expertName = profile?.name || localStorage.getItem('userName') || 'Expert';
    ...
    return (
        <div className="exp-page">
                ...
                <div>
                    ...
                    <p className="exp-subtitle">
                        {profile?.specialization
                            ...
                            : 'Complete your profile in Settings to help farmers find you.'}
                    </p>
                </div>
                <div style={inlineStyles.ratingPill}>
                    <Star size={14} fill="var(--exp-gold)" color="var(--exp-gold)" />
                    <span style={{ fontWeight: 700, color: 'var(--exp-gold)' }}>
                ...
                {statConfig.map(({ key, label, icon, color, suffix }) => (
                    <div key={key} className="exp-card interactive exp-stat-card">
                        ...
                        <div className="exp-stat-value" style={{ color }}>
                            <span>{typeof stats?.[key] === 'number' ? stats[key] : stats?.[key] ?? 0}</span>
            ...
            <div style={{ marginBottom: 28 }}>
                <div className="exp-section-title" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span>
                        Connection Requests
                        <span className="exp-count-badge">{connections?.length ?? 0}</span>
                    </span>
                    <Link to="/expert-dashboard/connections" style={{ fontSize: 13, color: 'var(--exp-green)', fontWeight: 600, textDecoration: 'none' }}>
                        View all →
                    </Link>
                </div>
                {(connections?.length ?? 0) === 0 ? (
                    <div className="exp-card" style={{ padding: '24px', textAlign: 'center', color: 'var(--exp-text-muted)', fontSize: 13 }}>
                        No pending connection requests. Farmers and customers will appear here when they connect with you.
                    </div>
                ) : (
                    connections.map(conn => {
                        ...
                        return (
                            <div key={conn.id} className="exp-card interactive exp-list-card" style={{ marginBottom: 10 }}>
                                <div className="exp-icon-box">
                                    {role === 'farmer' ? <Tractor size={16} /> : <ShoppingBag size={16} />}
                                </div>
                                <div style={{ flex: 1 }}>
                                    <div className="exp-list-title">{user?.name || 'Unknown User'}</div>
                                    <div className="exp-list-meta">
                                        <span className="exp-meta-item">{role === 'farmer' ? 'Farmer' : 'Customer'}</span>
                                        {user?.district && <span className="exp-meta-item">{user.district}</span>}
                                        <span>{timeAgo(conn.createdAt)}</span>
                                    </div>
                                </div>
                                <span className="exp-badge pending">pending</span>
                            </div>
                        );
                    })
                )}
            ...
            <div className="exp-two-col">
                <div>
                    ...
                    {(consultations?.length ?? 0) === 0 && (
                        <div style={{ color: 'var(--exp-text-muted)', fontSize: 13, padding: '20px 0' }}>
                        ...
                        <div key={q.id} className="exp-card interactive" style={{ marginBottom: 12, padding: '18px 22px' }}>
                            ...
                            <div className="exp-list-meta" style={{ marginBottom: 0 }}>
                                ...
                                <span style={{ color: q.replyCount > 0 ? 'var(--exp-green)' : 'var(--exp-text-muted)', fontWeight: 600 }}>
                                    {q.replyCount || 0} {q.replyCount === 1 ? 'reply' : 'replies'}
                                </span>
                            </div>
                        </div>
                    ))}
