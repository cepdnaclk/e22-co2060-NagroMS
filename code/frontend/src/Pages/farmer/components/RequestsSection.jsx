    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{
        ...
        <h1 style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: '28px', fontWeight: 700, margin: '0 0 8px 0', color: '#ffffff' }}>
          <FileText size={32} /> Incoming Product Requests
        </h1>
        <p style={{ fontSize: '16px', color: '#ccfbf1', margin: 0 }}>
          Respond to requests from customers looking for products you can supply
        </p>
      </div>

      {/* Main List */}
      <div style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '16px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)' }}>
        <h2 style={{ fontSize: '20px', color: '#115e59', marginBottom: '20px' }}>Active Customer Requests</h2>
        {requests.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '48px 0' }}>
            <Package size={64} style={{ color: '#9ca3af', marginBottom: '16px' }} />
            <p style={{ fontSize: '16px', color: '#6b7280', margin: 0 }}>No product requests are currently targeted at you.</p>
          </div>
        ) : (
              ...
              return (
                <div key={req.id} style={{
                  ...
                  <div style={{ display: 'flex', justifycontent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontSize: '14px', color: '#6b7280', margin: 0 }}>
                        Requested by <strong style={{ color: '#115e59' }}>{req.customerName}</strong> on {new Date(req.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                    <div>
                      {myResponse ? (
                  ...
                  <div style={{ padding: '16px', backgroundColor: '#f9fafb', borderRadius: '8px', fontSize: '14px', color: '#374151' }}>
                    <p style={{ margin: '0 0 8px 0' }}><strong>Quantity Needed:</strong> {req.quantity || 'N/A'}</p>
                    <p style={{ margin: 0 }}><strong>Description:</strong> {req.description || 'No description provided.'}</p>
                  </div>
                  ...
                  {myResponse && myResponse.message && (
                    <div style={{ fontSize: '13px', color: '#4b5563', fontStyle: 'italic', marginTop: '4px' }}>
                      Your message: "{myResponse.message}"
                    </div>
                  )}
                </div>
              );
        ...
        <div style={{
          ...
          <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', maxWidth: '480px', width: '100%', padding: '24px', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)' }}>
            <h3 style={{ fontSize: '20px', fontWeight: 700, color: '#115e59', marginBottom: '12px' }}>
              {actionStatus === 'accept' ? 'Accept Request' : 'Decline Request'}
            </h3>
            <p style={{ fontSize: '14px', color: '#6b7280', marginBottom: '16px' }}>
              {actionStatus === 'accept' 
                ? `Write a message to ${respondingTo.customerName} about availability, pricing, or delivery details.`
                : `Provide a reason to ${respondingTo.customerName} for declining this request.`}
            </p>
