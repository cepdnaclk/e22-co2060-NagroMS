// ============================================================
// NagroMS — Customer/ExpertsListSection.jsx
// Customers can view all registered Agricultural Experts
// ============================================================

import React, { useState, useEffect } from 'react';
import { GraduationCap, MapPin, Phone, Star, Search } from 'lucide-react';
import { subscribeToUsersByRole } from '../../../../../utils/userDirectoryService';

const initials = (name = '') =>
  name.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 2) || '?';

export default function ExpertsListSection() {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{
        ...
        <p style={{ margin: '8px 0 0', opacity: 0.9, fontSize: '15px' }}>
          Get professional advice from certified agricultural experts
        </p>
      </div>
      ...
      {!loading && filtered.length === 0 && (
        <div style={{ background: 'white', borderRadius: '12px', padding: '48px', textAlign: 'center', border: '1px solid #e5e7eb' }}>
          ...
          <p style={{ color: '#9ca3af', fontSize: '14px', margin: 0 }}>
            {search ? 'Try a different search term.' : 'No experts have registered yet.'}
          </p>
        </div>
      )}
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 700, fontSize: '15px', color: '#111827', marginBottom: '2px' }}>{expert.name}</div>
                    <div style={{ fontSize: '12px', color: '#6b7280' }}>{expert.specialty || 'Agricultural Expert'}</div>
                  </div>
                  <span style={{ background: '#e8f5e9', color: '#115e59', fontSize: '11px', fontWeight: 700, padding: '3px 10px', borderRadius: '20px', flexShrink: 0 }}>Expert</span>
                </div>

                <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                  {expert.district && <span style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', color: '#6b7280' }}><MapPin size={13} color="#115e59" />{expert.district}</span>}
                  {expert.phone && <span style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', color: '#6b7280' }}><Phone size={13} color="#115e59" />{expert.phone}</span>}
